                        ...
                        <CardContent className="space-y-4">
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                              <span className="text-success">For: {percentages.for.toFixed(1)}%</span>
                              <span className="text-destructive">Against: {percentages.against.toFixed(1)}%</span>
                            </div>
                            <div className="h-2 rounded-full bg-muted overflow-hidden flex">
                              <div className="bg-success h-full" style={{ width: `${percentages.for}%` }} />
                              <div className="bg-destructive h-full" style={{ width: `${percentages.against}%` }} />
                              <div className="bg-muted-foreground/30 h-full" style={{ width: `${percentages.abstain}%` }} />
                            </div>
                          </div>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input id="title" placeholder="Enter proposal title..."
                      value={newProposalTitle} onChange={(e) => setNewProposalTitle(e.target.value)}
                    />
                  </div>

                  <div className="space-y-2">
                    <Label htmlFor="description">Description</Label>
                    <Textarea id="description" placeholder="Describe your proposal in detail..."
                      value={newProposalDescription} onChange={(e) => setNewProposalDescription(e.target.value)} rows={6}
                    />
                  </div>

                  <Button className="w-full" onClick={() => createProposal.mutate()}
                  ...
                  <ScrollArea className="h-[500px]">
                    <div className="space-y-3">
                      {governanceData?.length === 0 && (
                        <p className="text-center py-8 text-muted-foreground">No governance indicator data available</p>
                      )}
                        ...
                        <div key={item.id} className="flex items-center justify-between p-3 border rounded-lg">
                          ...
                          <div className="text-right">
                            <p className={`text-lg font-bold ${
                              item.value > 1 ? 'text-success' : 
                              item.value > 0 ? 'text-warning' : 
                              'text-destructive'
                            }`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {spaces?.length === 0 && (
                  <Card className="p-8 text-center md:col-span-2">
                    ...
                    <p className="text-muted-foreground">No governance spaces configured</p>
                  </Card>
                )}
                {spaces?.map((space: any) => (
                  <Card key={space.id}>
