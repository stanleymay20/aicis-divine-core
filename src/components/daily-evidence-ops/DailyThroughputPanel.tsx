...
export default function DailyThroughputPanel() {
  ...
  const metrics = [
    ...
    { label: "Accepted Today", value: data?.acceptedToday ?? 0 },
    { label: "Started Today", value: data?.startedToday ?? 0 },
    { label: "Completed Today", value: data?.completedToday ?? 0 },
    { label: "Measured Today", value: data?.measuredToday ?? 0, highlight: true },
    { label: "Measured 7d", value: data?.measuredWeek ?? 0, highlight: true },
    { label: "Accept→Measured", value: data?.conversion ?? "0%" },
  ];
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <Badge variant={data?.measuredToday ? "default" : "secondary"} className="text-xs font-mono">
            {data?.measuredToday ?? 0} measured today
          </Badge>
        </div>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-2">
          {metrics.map(m => (
            <div key={m.label} className={`rounded-lg border p-2 text-center ${m.highlight ? "border-primary/30 bg-primary/5" : "border-border/30"}`}>
              <p className={`text-lg font-mono font-bold ${m.highlight ? "text-primary" : "text-foreground"}`}>{m.value}</p>
