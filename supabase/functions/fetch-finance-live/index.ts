...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Invoking pull-coingecko for live finance data...");
    
    const { data, error } = await supabase.functions.invoke('pull-coingecko', {
      body: {}
    });
    ...
    return new Response(
      JSON.stringify({ 
        ok: true, 
        message: "Finance data refreshed successfully",
        data
      }), 
